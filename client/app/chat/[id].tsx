...
export default function ChatScreen() {
  ...
  const pickMedia = async () => {
    ...
    if (status !== "granted") {
      Alert.alert("permission needed", "Allow media access to send images.");
      return;
    }
  ...
  const send = async () => {
    if ((!text.trim() && !mediaUri) || !selectedConversations) return;
    setSending(true);
    setTimeout(() => {
      setSending(false);
      setText("");
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.emptyState}>
          ...
          />
          <Text style={styles.emptyState}>COnversation not found!</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      >
        ...
        {loading ? (
          ...
          <FlatList
            ...
            onContentSizeChange={() =>
              flastListRef.current?.scrollToEnd({ animated: false })
            }
          />
        )}
